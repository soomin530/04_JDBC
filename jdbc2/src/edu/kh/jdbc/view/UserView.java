...
public class UserView {
	...
	private void selectName() throws Exception {
		...
		// 서비스 호출(SELECET) 후 결과(List<User>) 반환 받기
		// 이름에 검색어가 포함된 사람이 여러 명일 수 있으므로 List 쓰기
	...
	private void selectUser() throws Exception {
		...
		// == 일치하는 사용자가 있다면 딱 1행만 조ㅗ히된다
		// -> 1행의 조회 결과를 담기 위해서 사용하는 객체 == User DTO
		
		User user = service.selectUser(input);
		
		// 조회된 결과 없을 경우 "USER_NO가 일치하는 회원이 없습니다" 출력
		if (user != null) {
			System.out.println(user);
			return; // 있을 경우에만 수행할 수 있게 return
		} else System.out.println("USER_NO가 일치하는 회원이 없습니다");
	}
	...
	/**
	 * ID, PW가 일치하는 회원이 있을 경우(SELECT) 이름 수정(UPDATE)
	 */
	private void updateName() throws Exception {
		...
		if(userNo == 0) { // 조회 결과 없음 (1번부터 시작하기 때문에)
	...
	private void insertUser2() throws Exception {
		...
		while(true) {
			...
			// 입력받은 userId가 중복인지 검사(SELECT)하는 서비스 호출 후
